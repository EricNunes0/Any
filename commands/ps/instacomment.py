...
class cog_instacomment(commands.Cog):
    def __init__(self, bot):
        self.bot = bot

    @commands.command(name = "instacomment", aliases = ["ic", "commentinsta", "instagram", "insta"])
    ...
    async def instacomment(self, ctx, *, mensagem=None):
        try:
            ...
            backgroundImage = requests.get("https://i.postimg.cc/ht11rcj2/Insta-Comment.png")
            ...
            psEmbed.set_image(url = "attachment://img_instacomment.png")
            psEmbed.set_author(name = f"『📷』Insta Comment:", icon_url = self.bot.user.display_avatar.url)
            psEmbed.set_footer(text = f"Pedido por {ctx.author.name}", icon_url = ctx.author.display_avatar.url)
            await ctx.send(embed = psEmbed, file=discord.File('img_instacomment.png'))
        except Exception as e:
            print(e)

    
async def setup(bot):
    print("cog_instacomment.py loaded")
    await bot.add_cog(cog_instacomment(bot))